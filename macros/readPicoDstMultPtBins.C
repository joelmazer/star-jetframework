...
// select run year
Int_t RunYear = 12;      // (17) 14 for 2014 AuAu
...
bool doSetupQA = kTRUE; //kFALSE; //kTRUE; // FIXME
...
void readPicoDstMultPtBins(const Char_t *inputFile="Run14_P18ih_HPSS_15164046.list", const Char_t *outputFile="test.root", Int_t nEv = 10, const Char_t *fEPoutJobappend="")
{
...
//  Int_t nEvents = 4000;
  ...
  enum RunType_t {
    ...
    mRun07 =  7, mRun08 = 8,  mRun09 = 9, mRun10 = 10, mRun11 = 11, mRun12 = 12, mRun13 = 13, mRun14 = 14, mRun15 = 15, mRun16 = 16, mRun17 = 17, mRun18 = 18, mRun19 = 19, mRun20 = 20
  };
  ...
  // BIpt2_scheme - standard scheme for jet reco
  enum ERecoScheme_t {
  ...
  bool doSkip1ParticleJets = kFALSE;            // only relevent to jet shape analyses
  bool doRequireJetTowFireTrig = kFALSE;         // require jet contain tower firing trigger
  bool doBiasJSjet = kFALSE;                     // jet shape jets: require leading bias (requiring tow firing trig kinda overrides this)
  //double fJetRadius = 0.4;  // 0.4, 0.3, 0.2  // set up top, as this is often overlooked when submitting multiple submission in a row
  double fJetConstituentCut = 2.0; //2.0;              // correlation analysis: 2.0, jet shape analysis: 1.0 (been using 2.0 for corrections)
  Int_t fJetAnalysisJetType = kLeadingJets;     // Jet analysis jet types - options: kInclusiveJets, kLeadingJets, kSubLeadingJets, USER needs to define and use w/ setter in their class
  ...
  //jetTask->SetJetHadCorrType(StJetFrameworkPicoBase::kHighestEMatchedTrack); // options:  kLastMatchedTrack, kHighestEMatchedTrack, kAllMatchedTracks
  ...
  if(!doSetupQA) {
    ...
    for(int i = 0; i < 9; i++) { // jet shape analysis
      ...
      anaMaker[i]->SetnEPBins(4);             // number of event plane bins to use for event mixing (0, pi) range
      if(dopp) { 
      ...
      anaMaker[i]->SetJetMaxTrackPt(4.0);   // jet track bias
...
void LoadLibs()
{
  ...
  // add include path to use its functionality - FIXME you should update this 
