...
// Adapted from the ALIROOT class AliAODBContainer
...
ClassImp(StCalibContainer);

//______________________________________________________________________________
StCalibContainer::StCalibContainer() : 
  TNamed(),
  fArray(0),
  fDefaultList(0),
  fPassNames(0),
...
StCalibContainer::StCalibContainer(const char* name) : 
  TNamed(name, "CalibContainer"),
...
//______________________________________________________________________________
StCalibContainer::StCalibContainer(const StCalibContainer& cont) :
  TNamed(cont),
  fArray(cont.fArray),
...
//______________________________________________________________________________
StCalibContainer& StCalibContainer::operator=(const StCalibContainer& cont)
{
  //
  // Assignment operator
...
void StCalibContainer::AppendObject(TObject* obj, Int_t lower, Int_t upper, TString passName)
{
...
void StCalibContainer::RemoveObject(Int_t idx)
{
  if (!fPassNames) { // create array of pass names for compatibility with old format
    fPassNames = new TObjArray(100);
    for (Int_t i=0;i<fArray->GetEntriesFast();i++) fPassNames->Add(new TObjString(""));
...
void StCalibContainer::UpdateObject(Int_t idx, TObject* obj, Int_t lower, Int_t upper, TString passName)
{
  //
...
void  StCalibContainer::AddDefaultObject(TObject* obj)
{
  // Add a default object
  fDefaultList->Add(obj);
}

void  StCalibContainer::CleanDefaultList()
{
  // Clean default list
  fDefaultList->Delete();
}

Int_t StCalibContainer::GetIndexForRun(Int_t run, TString passName) const
{
  //
...
TObject* StCalibContainer::GetObject(Int_t run, const char* def, TString passName) const
{
...
TObject* StCalibContainer::GetObjectFromFile(TFile* file, Int_t run, const char* def, TString passName) const
{
  // Return object for given run or default if not found
  TObject* obj = 0;
  Int_t idx = GetIndexForRun(run, passName);
...
TObject* StCalibContainer::GetObjectByIndex(Int_t run) const
{
  // Return object for given index
  return (fArray->At(run));
}

TObject* StCalibContainer::GetPassNameByIndex(Int_t idx) const
{
  // Return object for given index
...
void StCalibContainer::WriteToFile(const char* fname) const
{
  //
...
Int_t StCalibContainer::InitFromFile(const char* fname, const char* key)
{
  ...
  if(!fname){ return 1; } //AliError("Can not expand path name");return 1;}
...
Int_t StCalibContainer::HasOverlap(Int_t lower, Int_t upper, TString passName) const
{
  //
...
void StCalibContainer::Browse(TBrowser *b)
{
   // Browse this object.
...
//______________________________________________________________________________
const char* StCalibContainer::GetCalibPath()
{
// returns the path of the Calibrations
// this static function just depends on environment variables

   static TString CalibPath;

   // FIXME need to fix this!!!
   if (gSystem->Getenv("OADB_PATH"))
      CalibPath = gSystem->Getenv("OADB_PATH");
   else if (gSystem->Getenv("ALICE_ROOT"))
      CalibPath.Form("%s/OADB", gSystem->Getenv("ALICE_ROOT"));
   else
   ::Fatal("StCalibContainer::GetCalibPath", "Cannot figure out AODB path. Define ALICE_ROOT or OADB_PATH!");
   return CalibPath;
}
